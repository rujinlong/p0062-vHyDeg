...
mod_vhydeg_ui <- function(id){
  ns <- NS(id)
  tagList(
    sidebarLayout(
      # Side panel
      sidebarPanel(
       sliderInput(ns("bitscore"), "Minimum bitscore", min = 50, max = 650, value = 300, step=10),
       uiOutput(ns("hydg")),
       width = 3
      ),
      ...
      mainPanel(
       tabsetPanel(
         ...
         tabPanel("NCBI",
                  shinycssloaders::withSpinner(reactableOutput(ns("tbl_ncbi")), type=8)
         ),
         tabPanel("PHROGs",
...
#' vhydeg Server Functions
#'
#' @noRd
mod_vhydeg_server <- function(id){
  moduleServer( id, function(input, output, session){
    ...
    output$tbl_ncbi <- renderReactable({
      df_ncbi_parent <- df_ncbi() %>%
        dplyr::select(-c(ipr_db_name, ipr_db_acc, ipr_db_desc, ipr_acc, ipr_desc, ipr_evalue)) %>%
        dplyr::distinct()

      df_ncbi_children <- df_ncbi() %>%
        dplyr::select(c(gene_id, ipr_db_name, ipr_db_acc, ipr_db_desc, ipr_acc, ipr_desc, ipr_evalue)) %>%
        dplyr::distinct()

      reactable(df_ncbi_parent, searchable = TRUE,
                defaultPageSize = 10,
                sortable = TRUE,
                filterable = TRUE,
                highlight = TRUE,
                ...
                columns = list(
                  gene_id = colDef(minWidth = 300, cell = function(value, index) {
                    url <- sprintf('https://www.ncbi.nlm.nih.gov/protein/%s/', value)
                    htmltools::tags$a(href = url, target = "_blank", as.character(value))
                  }),

                  genome_id = colDef(minWidth = 200, cell = function(value, index) {
                    url <- sprintf('https://www.ncbi.nlm.nih.gov/nuccore/%s', value)
                    htmltools::tags$a(href = url, target = "_blank", as.character(value))
                  }),
                ...
                details = function(index) {
                  children = df_ncbi_children %>%
                    dplyr::filter(gene_id == df_ncbi_parent$gene_id[index]) %>%
                    dplyr::select(-gene_id)
                  tbl <- reactable(children, resizable = T, wrap = F, highlight = T, sortable = T, filterable = T)
                  htmltools::div(style = "padding: 1rem", tbl)
                }
